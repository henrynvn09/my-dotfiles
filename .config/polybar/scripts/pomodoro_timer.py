...
WORK_ACTIVE_PREFIX = u"%{F#a2ffff}%{T2}%{T-}%{O4}%{T2}"
WORK_ACTIVE_SUFFIX = u"%{T-}%{F-}"
WORK_INACTIVE_PREFIX = u"%{F#5b6268}%{T2}%{T-}%{O4}%{T2}"
WORK_INACTIVE_SUFFIX = u"%{T-}%{F-}"

BREAK_ACTIVE_PREFIX = u"%{F#98be65}%{T2}%{T-}%{O4}%{T2}"
BREAK_ACTIVE_SUFFIX = u"%{T-}%{F-}"
BREAK_INACTIVE_PREFIX = u"%{F#5b6268}%{T2}%{T-}%{O4}%{T2}"
            ...
            try:
                call(
                    [
                        ...
                        "5000",
                        "-i",
                        "~/.config/polybar/scripts/pomodoro-timer.png",
                        "-u",
                        "normal",
                        "Pomodoro Timer",
                        Status_String + " is over.",
                    ],
                    stdout=DEVNULL,
                    stderr=DEVNULL,
                )
                filename = "/home/ying/.config/polybar/scripts/pomodoro-sound.wav"
...
class Status:
    def __init__(self, worktime, breaktime, longbreaktime):
    ...
    def next_timer(self):
        if self.status == "work":
            ...
            self.cyclecounts += 1
            if self.cyclecounts == 4:
                self.timer = Timer(self.longbreaktime)
                self.cyclecounts = 0
                Status_String = "Long Break"
            else:
                self.timer = Timer(self.breaktime)
                Status_String = "Short Break"
...
def action_display(args):
    ...
    status = Status(args.worktime, args.breaktime, args.longbreaktime)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=15 * 60,
        help="Default long break timer time in seconds",
    )
